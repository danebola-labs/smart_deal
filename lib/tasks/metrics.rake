...
namespace :metrics do
  ...
  task :create_sample_query, %i[model_id input_tokens output_tokens] => :environment do |_t, args|
  ...
  task :collect_range, %i[start_date end_date] => :environment do |_t, args|
...
namespace :debug do
  ...
  task :verify_bucket, [:bucket_name] => :environment do |_t, args|
    ...
    begin
      ...
      kb_indicators << if total_count.positive?
                         "✅ Contains #{total_count} objects"
                       else
                         '⚠️  Empty bucket'
                       end

      kb_indicators << "✅ Contains #{number_to_human_size(total_size)} of data" if total_size.positive?
  ...
  task find_kb_bucket: :environment do
    ...
    begin
      ...
      buckets.each do |bucket|
        ...
        next unless bucket_name.downcase.include?('bedrock') ||
                    bucket_name.downcase.include?('knowledge') ||
                    bucket_name.downcase.include?('kb') ||
  ...
  task complete: :environment do
    ...
    kb_id = Rails.application.credentials.dig(:bedrock,
                                              :knowledge_base_id) || ENV.fetch('BEDROCK_KNOWLEDGE_BASE_ID', nil)
    puts "Knowledge Base ID: #{kb_id || '❌ Not configured'}"

    model_id = Rails.application.credentials.dig(:bedrock,
  ...
  task aurora: :environment do
    ...
    begin
      ...
      if cluster.status == 'available' && cluster.capacity && cluster.capacity.positive?
        puts "   ✅ Cluster is ACTIVE with #{cluster.capacity} ACUs"
      elsif cluster.status == 'available' && (!cluster.capacity || cluster.capacity.zero?)
