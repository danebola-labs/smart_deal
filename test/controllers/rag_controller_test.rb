...
class RagControllerTest < ActionDispatch::IntegrationTest
  ...
  def create_mock_service(answer:, citations: [], session_id: TEST_SESSION_ID_1, should_raise: false,
                          error_message: nil)
    mock_service = Object.new
    mock_service.define_singleton_method(:query) do |_question|
  ...
  test 'validates complete JSON response structure' do
    ...
    with_mock_bedrock_rag_service(mock_service) do
      ...
      required_keys = %w[status answer citations session_id]
